            ...
            "content": [
                {
                    "type": "text",
                    "text": "",
                },  # you can condition the input by the text prefix, it's empty string now
            ],
        },
    ]
    return messages
...
inputs = (
    input()
)  # give it like: echo "model_name,input_file_name,photos_dir_name,output_file_name" | python script_name.py
...
for index, row in input_dataset[output_dataset_length:].iterrows():
    ...
    out = pipe(text=messages, max_new_tokens=500, return_full_text=False)[0][
        "generated_text"
    ]
